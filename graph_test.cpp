/*
   ...
   Explain and justify how you approached testing, the degree
    ...
    implicitly included. Complex cases are ignored for simplicity. These tests
 ...
 GIVEN("a graph with edges") {
   ...
   WHEN("getting the connections of a node with no edges") {
     ...
     THEN("an empty vector is returned") {
       REQUIRE(connections == std::vector<int>{});
     }
   }
 }
}
...
SCENARIO("iterating") {
  GIVEN("an iterator to the beginning of a graph") {
    ...
    WHEN("dereferencing the iterator") {
      ...
      THEN("auto tuple of node from, node to, and edge weight is returned") {
        REQUIRE(src == "B");
        REQUIRE(dst == "B");
        REQUIRE(cst == 5.0);
      }
    ...
    WHEN("pre-incrementing the iterator") {
      ...
      THEN("the next tuple of node from, node to and edge weight is returned ordered by node from then node to then edge weight") {
    ...
    WHEN("iterating past the last edge") {
      ...
      THEN("the end iterator is reached") {
        REQUIRE(it == g.end());
      }
    }
  }
